...
data:
  ...
  bundledCode: "#line 2 \"structure/persistentlazysegtree.hpp\"\n#include <bits/stdc++.h>\n\
    ...
    \ (int)root.size() - 1;\n  }\n\n  S get(int ver, int p) { return prod(ver, p,\
    ...
    \  int build_empty(int l, int r) {\n    if (r - l == 1) return new_node(e(), id(),\
    \ -1, -1, false);\n    int m = (l + r) >> 1;\n    int lc = build_empty(l, m);\n\
    \    int rc = build_empty(m, r);\n    return new_node(op(nd[lc].val, nd[rc].val),\
    \ id(), lc, rc, false);\n  }\n\n  int build_vec(int l, int r, const vector<S>&\
    ...
    \ m, r, ql, qr, f);\n    nd[v].l = lc;\n    nd[v].r = rc;\n    nd[v].val = op(nd[lc].val,\
    \ nd[rc].val);\n    return v;\n  }\n\n  int set(int v, int l, int r, int p, S\
    \ x) {\n    v = clone(v);\n    if (r - l == 1) {\n      nd[v].val = x;\n     \
    \ nd[v].lz = id();\n      nd[v].has = false;\n      return v;\n    }\n    push(v,\
    \ l, r);\n    int m = (l + r) >> 1;\n    if (p < m) {\n      nd[v].l = set(nd[v].l,\
    ...
    \    int m = (l + r) >> 1;\n    F nxt = acc;\n    if (nd[v].has) nxt = composition(acc,\
  ...
  requiredBy: []
  timestamp: '2026-03-11 22:30:57+09:00'
